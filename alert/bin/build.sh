...
aws s3 cp $1/$PRO_NAME/bin/server.sh s3://fcasb-data/devops/$PRO_NAME/bin/server.sh
aws s3 cp $1/$PRO_NAME/bin/install.sh s3://fcasb-data/devops/$PRO_NAME/install.sh
